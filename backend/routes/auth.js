...
router.post('/login', (req, res) => {
  ...
  return res.json({ token, user: { email, is_pro: !!u.isPro, trial_days_left: trialDaysLeft(email) } });
});
...
router.post('/register', (req, res) => {
  ...
  const token = signToken(email);
  return res.json({ token, user: { email, is_pro: false, trial_days_left: 7 } });
});

// demo upgrade fallback (kept)
